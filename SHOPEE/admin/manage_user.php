<?php 
  ...
  if(isset($_GET["task"]) && $_GET["task"] == "delete") {
    $id = $_GET["id"];
    $sql_delete = "delete from information where id = " . $id;
  ...
  if(isset($_POST["delete_check"])) {
    if(isset($_POST["cate"])) {
      ...
      foreach($cate as $c) {
        echo $c;
        $sql_delete = "delete from information where id = " . $c;
        if(mysqli_query($conn, $sql_delete)) {
          echo "Xoa thanh cong";
          header("location:category.php");
        } else {
  ...
  <body>
    <div class="container">
      ...
      <div class="row" style="margin-top: 10px; display: flex;">
        <div class="col-6">
          <form action="category.php" method="post" style="margin-top: 10px; display: flex;">
              ...
              if (mysqli_num_rows($result)>0)
              {
                while ($row = mysqli_fetch_assoc($result))
                {
                  ...
                  echo "<td>";
                    ...
                    echo "<a class='btn btn-danger' href='category.php?task=delete&ID=".$row["ID"]."'> Xoa</a>";
                  echo "</td>";
